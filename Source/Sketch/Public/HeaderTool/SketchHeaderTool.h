...
namespace sketch::HeaderTool
{
	...
	//
	// 	/// Grouping
	// 	void BeginFile(const sketch::FStringView& FileName) { InitFile(FileName), OnNewFile.Broadcast(); }
	// 	void BeginClass(const sketch::FStringView& ClassName) { InitClass(Messages.Get(CurrentFile).Value, ClassName); }
	// 	void BeginSlot(const sketch::FStringView& SlotName) { InitSlot(Messages.Get(CurrentFile).Value.Get(CurrentClass).Value, SlotName); }
	// 	void EndSlot() { End(Messages.Get(CurrentFile).Value.Get(CurrentClass).Value, CurrentSlot), CurrentSlot = Messages.Get(CurrentFile).Value.Get(CurrentClass).Value.FindId({}); }
	// 	void EndClass() { CurrentClass = Messages.Get(CurrentFile).Value.FindId({}), EndSlot(); }
	// 	void EndFile() { CurrentFile = Messages.FindId({}), EndClass(); }
	// 	void Reset() { Messages.Reset(), InitFile({}), OnReset.Broadcast(); }
	//
	// 	/// Logging
	// 	FORMATTER(2, 4)
	...
	// 		Messages.Get(CurrentFile).Value.Get(CurrentClass).Value.Get(CurrentSlot).Value.Emplace(Verbosity, SourceLine, FString::Printf(UncheckedFormat, Args...));
	...
	// 	/// Contents
	// private:
	// 	void InitFile(const sketch::FStringView& FileName)
	// 	{
	// 		const uint32 Hash = GetTypeHash(FileName);
	// 		Messages.EmplaceByHash(Hash, FileName.ToCommonStringView());
	// 		CurrentFile = Messages.FindIdByHash(Hash, FileName.ToCommonStringView());
	// 		InitClass(Messages.Get(CurrentFile).Value, {});
	// 	}
	//
	// 	void InitClass(TMap<FString, TMap<FString, TArray<FMessage>>>& File, const sketch::FStringView& ClassName)
	// 	{
	// 		const uint32 Hash = GetTypeHash(ClassName);
	// 		File.EmplaceByHash(Hash, ClassName.ToCommonStringView());
	// 		CurrentClass = File.FindIdByHash(Hash, ClassName.ToCommonStringView());
	// 		InitSlot(File.Get(CurrentClass).Value, {});
	// 	}
	//
	// 	void InitSlot(TMap<FString, TArray<FMessage>>& Class, const sketch::FStringView& SlotName)
	// 	{
	// 		const uint32 Hash = GetTypeHash(SlotName);
	// 		Class.EmplaceByHash(Hash, SlotName.ToCommonStringView());
	// 		CurrentSlot = Class.FindIdByHash(Hash, SlotName.ToCommonStringView());
	// 	}
	//
	// 	static void End(auto& Map, FSetElementId Id)
	// 	{
	// 		if (Id.IsValidId() && Map.Get(Id).Value.IsEmpty())
	// 		{
	// 			Map.Remove(Id);
	// 		}
	// 	}
	//
	// 	TMap<FString, TMap<FString, TMap<FString, TArray<FMessage>>>> Messages;
	// 	FSetElementId CurrentFile;
	// 	FSetElementId CurrentClass;
	// 	FSetElementId CurrentSlot;
	// };
