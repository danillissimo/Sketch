 ...
 * @note Initially sketch::TTuple, but renamed to avoid collisions with any other tuples.
 */
template <class ItemType = void, class... ItemTypes>
struct TSketchTuple
{
	constexpr TSketchTuple()
	{
	...
	constexpr TSketchTuple(ItemType FirstItem, ItemTypes... Items)
		: Item(FirstItem)
		, Next(Items...)
	{}

	template <class ItemInitializerType, class... ItemInitializerTypes>
	constexpr TSketchTuple(ItemInitializerType FirstItem, ItemInitializerTypes... Items)
		: Item(FirstItem)
		, Next(Items...)
	{
	...
	{
		TSketchTuple<decltype(Args)...> PackedArgs{ std::forward<ArgTypes>(Args)... };
...
private:
	...
	constexpr auto ApplyBeforeImpl(CallableType&& Callable, TSketchTuple<ArgTypes...>& PackedArgs, PreviousItemsTypes&... PreviousItems)
	{
		if constexpr (sizeof...(ItemTypes) == 0)
		{
...
template <>
struct TSketchTuple<void>
{};

template <class... ArgTypes>
constexpr TSketchTuple<std::decay_t<ArgTypes>...> MakeSketchTuple(ArgTypes&&... Args) { return TSketchTuple<std::decay_t<ArgTypes>...>(std::forward<ArgTypes>(Args)...); }

template <class... ArgTypes>
constexpr TSketchTuple<ArgTypes&...> SketchTie(ArgTypes&... Args) { return TSketchTuple<ArgTypes&...>(Args...); }
