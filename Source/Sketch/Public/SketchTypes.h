...
namespace sketch
{
	...
	struct FAttribute : public TSharedFromThis<FAttribute>
	{
		...
		using FCustomSloteCodeGenerator = FString(*)(const FSlotBase*);
		using FMetaValue = TVariant<FEmptyVariantState, int64, FStringView, FCustomSloteCodeGenerator>;
	...
	struct TAttributeInitializer
	{
		...
		TAttributeInitializer& AddMeta(const FName& Key, FAttribute::FCustomSloteCodeGenerator CodeGenerator) { return Meta.Add(Key, FAttribute::FMetaValue(TInPlaceType<FAttribute::FCustomSloteCodeGenerator>{}, CodeGenerator)), *this; }
